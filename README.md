...
直接放着 `NN.md`,这个目录就是一个可选课程。页面上"课程"这里会按
嵌套层级显示**多个级联下拉框**——选第一层(比如课程大类),第二个
下拉框会跟着刷新成这一层下面实际有的子文件夹,一直到选到真正存放
`NN.md` 的那一层为止;每往深一层都是一次页面刷新,重新显示对应内容
的预览。不要求所有课程嵌套层数一样深,例如:
...
上面这个例子第一层下拉框会有 4 个选项("OCI课程包_对照AWS"、
"k8s-course"、"托业单词"、"网络监控四层通信方式解析"),选中某一个后
第二层下拉框会显示它下面的子文件夹("中文版"/"日文版"/"英语版",或者
"托业单词"下面就只有一个"20260630")。选到最后一层后,`NN.conversation`
就是这个目录下的内容,不需要靠文件夹名字猜是哪种语言,也不需要再额外
加 `.zh`/`.ja`/`.en` 后缀(除非你就是想在同一个目录里用后缀区分多语言,
见下面的说明)。

页面首次打开时默认选中的课程可以在 `config.py` 的 `DEFAULT_COURSE_HINT`
里改(目前设的是 `"托业单词"`,会优先选中路径第一层等于这个值的课程,
找不到就用列表里第一个)。
...
- **播放并录屏**:开始录屏 + 依次播放 + 结束后停止录屏,用于正式产出
  视频,需要按前面第二步配置好 ffmpeg 录屏设备。
- **仅播放(不录屏)**(默认):只依次显示流程图、播放语音、显示字幕,
  完全不启动 ffmpeg,适合还没配置好录屏设备时快速预览素材、检查
  字幕/发音效果,或者单纯调试用。正式录制前记得手动切换成"播放并录屏"。
...
再旁边是"字幕字号"滑块(默认 28px,范围在 `config.py` 的
`CAPTION_FONT_SIZE_*` 里改),这个是纯前端效果,拖动立刻在画面上
生效,不需要重新生成语音。字幕本身没有背景色的黑框了,改成白色描边
让文字在各种底色的画面上都还能看清;播放/录制过程中字幕还可以直接
用鼠标按住拖到画面上任意位置,松手后位置会一直保持,直到你再拖一次
或者重新开始下一轮播放。
...
6. 视频保存在 `output/` 文件夹里,文件名会自动加上语言/章节区间/时间戳,
   ...
   生成新文件,不会覆盖之前录的
7. "课程"下拉框下面有个"清空缓存和视频"按钮,点了会弹窗确认,确认后
   会删掉 `tts_cache/` 里所有缓存的语音文件和 `output/` 里所有已经录好
   的视频(**不可恢复**,需要保留的视频记得提前备份出去)。如果当前
   正有播放/录制在进行中,点了会被拒绝并提示先停止,避免删到正在
   写入的文件。
